...
STEP="复制文件系统"
merge_local_fs danmakurecord

STEP="更新配置"
buildah_config danmakurecord --entrypoint '["/entrypoint.sh"]' --cmd 'init' \
	--volume=-/rec --volume=/data/raw --volume=/data/mp4

RESULT=$(create_if_not "danmakurecord" "$BUILDAH_LAST_IMAGE")
buildah commit "$RESULT" gongt/danmakurecord
