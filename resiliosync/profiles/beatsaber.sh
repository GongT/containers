...
function ensure_symlink() {
	local LINK="$1"
	local TARGET="$2"
	log "create link '$LINK'"

	if [[ -L $LINK ]]; then
		local CURR="$(realpath "$LINK")"
		log "  - found exists link to $CURR"
		if [[ $CURR != "$TARGET" ]]; then
			unlink "$LINK"
		else
			return 0
		...
		if [[ -d $LINK ]] && rmdir "$LINK"; then
			log "  - removed empty directory: $LINK"
		else
			log "ERROR: $LINK exists but not symlink"
			exit 66
		fi
	fi

	log "  - creating symlink to $TARGET"
...
function process_line() {
	...
	if [[ $(echo -n "$TITLE_SECRET" | wc -c) -ne 33 ]]; then
		log "==================================="
		log "Invalid Line: hash not equals to 33"
		log "$line"
		log "==================================="
	fi
